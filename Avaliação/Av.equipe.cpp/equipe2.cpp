...
void RegistrarOcorrenciaPorVeiculo(std::vector<Locacao> &locacoes)
{
    std::string placaVeiculo;
    std::cout << "Informe a Placa do Veículo para registrar a ocorrência: ";
    std::cin >> placaVeiculo;
    bool ocorrenciaRegistrada = false;

    for (Locacao &locacao : locacoes)
    {
        if (locacao.veiculo.Placa == placaVeiculo)
        {
            ...
            std::getline(std::cin, novaOcorrencia.Data_Hora);
            std::cout << "Informe o Número da Apólice: ";
            std::cin >> novaOcorrencia.Numero_Apolice;

            locacao.ocorrencia = novaOcorrencia;
            ocorrenciaRegistrada = true;

            std::cout << "Ocorrência registrada com sucesso para o veículo de placa: " << placaVeiculo << std::endl;
        }
    }

    if (!ocorrenciaRegistrada)
    {
        std::cout << "Nenhum veículo encontrado com a placa informada." << std::endl;
    }
}
...
int main()
{
    ...
    do
    {
        ...
        std::cout << "5. Registrar Ocorrência por Veículo" << std::endl;
        ...
        case 4:
            ...
            break;
        case 5:
            RegistrarOcorrenciaPorVeiculo(locacoes);
            break;
        case 0:
